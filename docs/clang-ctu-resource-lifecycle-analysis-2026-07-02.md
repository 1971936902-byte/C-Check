...
在 POC 复核后增加以下保护，避免静态分析层影响已有优质结果：

- C 文件数超过配置上限时不再静默截取前若干文件，而是明确标记为未完成并回退原流程。
- Clang 诊断保留编译器给出的原始行号，不再经过面向 LLM 候选的行号重定位。
- LLM 与 Clang 对同一资源、同一函数或相邻位置的发现进行证据聚合，编译器结论优先；不同资源不会合并。
- CodeChecker 使用 `--disable-all` 后仅开启资源生命周期相关 checker，避免默认 core checker 扩大报告噪声。
- CTU 默认使用一个分析 job，且遵守配置的超时，不再强制至少等待 120 秒。
- 外部分析通过工作线程执行，避免同步阻塞异步候选整理流程。
- 临时编译命令加入所有上传源码和头文件所在目录，覆盖常见的项目内相对 include。
- `CodeChecker` 作为 `static-analysis` 可选依赖，原生部署脚本同时安装 Clang 和 clang-tools，确保环境可复现。
...
1. `ctu_stress`
   - 两个源文件和一个头文件。
   - 包含跨文件自定义资源、结构体成员持有资源、标准流句柄、double-free 和 use-after-free。
   - 金标漏洞 5 个，安全函数 3 个。
   - 结果：5/5 命中、0 误报、0 漏报，精确率和召回率均为 100%。
   - 单次耗时：1.3083 秒。
2. `nested_include`
   - 头文件位于 `include/`，实现和调用者位于 `src/`。
   - 验证自动加入源码与头文件目录后，跨目录 include 与 CTU 调用解析正常。
   - 结果：唯一泄漏命中，安全函数未报告；耗时 1.2655 秒。

稳定性测试结果：

| 场景 | 次数 | 平均耗时 | 最大观测耗时 | 全部完成 | 诊断一致 |
|---|---:|---:|---:|---|---|
| 顺序重复 | 5 | 1.2794 秒 | 1.3216 秒 | 是 | 是 |
| 三路并发 | 3 | 1.3210 秒 | 1.3279 秒 | 是 | 是 |

三路并发相对顺序平均耗时增加约 3.3%，所有运行均返回相同的 5 条诊断且无错误。收口后的完整回归结果为：

- 云服务器：`222 passed, 1 deselected, 3 warnings`。被排除项仍是线上 `.env` 覆盖测试默认值的问题。
- 本地 Windows：`220 passed, 3 skipped, 3 warnings`。跳过项均为需要真实 Clang/CodeChecker 的 Linux 验收测试。
