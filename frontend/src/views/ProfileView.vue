...
import { validatePasswordChange } from './form-validation'
const form = reactive({ current: '', next: '', confirm: '' }), loading = ref(false), auth = useAuthStore(), router = useRouter()
async function submit() { const message = validatePasswordChange(form.current, form.next, form.confirm); if (message) return ElMessage.warning(message); loading.value = true; try { await authApi.password(form.current, form.next); auth.logout(); ElMessage.success('密码已修改，请重新登录'); router.push('/login') } catch (e) { ElMessage.error(errorMessage(e)) } finally { loading.value = false } }
</script>
