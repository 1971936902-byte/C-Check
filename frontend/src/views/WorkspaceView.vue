...
const source = ref(''), singleFile = ref<File>(), archiveFile = ref<File>(), task = ref<ReviewTask>(), submitting = ref(false), router = useRouter()
...
const fileProgress = computed(() => task.value ? deriveFileProgress(task.value) : [])
...
<aside class="panel glass task-panel"><div class="task-panel-header"><div><h2>任务状态</h2><p v-if="!task">提交代码后，可在此查看审查进度与结果。</p></div><el-button v-if="MOCK_API_ENABLED && !task" plain @click="runDemo">加载多文件演示</el-button></div><template v-if="task"><div class="task-title"><div><strong>{{ task.display_name }}</strong><small>{{ task.file_count }} 个文件 · {{ task.check_types?.length || 0 }} 项检查</small></div><StatusBadge :status="task.status" /></div><el-progress :percentage="task.progress" :status="task.status === 'failed' ? 'exception' : task.status === 'completed' ? 'success' : undefined" /><div class="task-detail-grid"><dl><div><dt>发现问题</dt><dd>{{ task.finding_count }}</dd></div><div><dt>耗时</dt><dd>{{ task.duration_ms ? `${(task.duration_ms / 1000).toFixed(1)}s` : '--' }}</dd></div></dl><section><h3>文件审查进度</h3><div class="file-progress-list"><div v-for="file in fileProgress" :key="file.id" class="file-progress-item"><i :class="`file-state file-state-${file.status}`"></i><code>{{ file.relative_path }}</code><small>{{ file.status === 'completed' ? '已完成' : file.status === 'analyzing' ? '分析中' : '待处理' }}</small></div></div></section></div><el-alert v-if="task.error_message" :title="task.error_message" type="error" :closable="false" show-icon /><el-button v-if="task.status === 'completed'" type="primary" class="report-button" @click="openReport">查看审查报告</el-button></template></aside></div></section></template>
