...
## 9. 2026-06-29 至 2026-06-30 第一阶段召回与 RAG 作用域调试

### 9.1 测试基线

本轮使用服务器 `/opt/c-check-testdata/Damn_Vulnerable_C_Program/linux` 下三个高密度漏洞文件作为同一批输入：

- `imgRead.c`
- `imgRead_libfuzzer.c`
- `imgRead_socket.c`

固定条件为 Qwen2.5-Coder-14B-Instruct、12,288 tokens 上下文、温度 0、第一阶段 RAG + JSONL，且不调用第二阶段。人工金标按 41 个独立漏洞触发点统计，覆盖整数溢出/下溢、危险复制、double-free、use-after-free、除零、栈/堆越界读写、泄漏和资源耗尽。

### 9.2 关键实验结果

| 方案 | 输出预算 | 候选数 | 触发点召回率 | 明确误报 | 耗时 | 完成原因 |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| 旧激活提示词 | 1024 | 24 | 58.54% | 0 | 25.10s | stop |
| 最新精简提示词 | 1024 | 27 | 60.98% | 约 1 | 28.51s | length |
| 精简提示词 | 2048 | 33 | 85.37% | 1 | 38.17s | stop |
| 单独强调 memcpy | 2048 | 27 | 75.61% | 0 | 28.49s | stop |
| 通用独立 callsite | 2048 | 38 | 92.68% | 0 | 41.94s | stop |
| 清理伪全局变量后仍使用 callsite | 2048 | 37 | 78.05% | 4 | 40.15s | stop |
| 通用独立 trigger location | 2048 | 39 | 92.68% | 0 | 39.38s | stop |

结论：

1. 第一阶段 1024 tokens 会截断高密度多文件结果；独立提高到 2048 后，三文件可以完整结束，第二阶段仍保持 1024，兼顾召回与格式化成本。
2. 单独强调 `memcpy` 会把命中从 3/9 提升到 6/9，但会挤掉整数问题，属于明显的注意力偏置，不适合作为通用模板。
3. 使用“独立漏洞触发位置”而不是“独立调用点”，并明确覆盖 executable statement or expression，可同时恢复算术表达式、生命周期和数组访问问题。
4. 最终模板在干净 RAG 上恢复至 38/41 触发点，当前主要漏报仍是每个文件第三处 `buff4 memcpy`。
5. 三份高度相似源码同时输入时，`imgRead_socket.c` 前半段出现过系统性行号偏移；后端行号重定位与按文件隔离定位仍需继续加强。

### 9.3 RAG 伪全局变量根因与修复

旧索引把 `size1/size2/size3` 等函数局部变量注入 Definition Context，原因是：

- 正则回退解析器没有判断花括号作用域，把函数体声明识别成 `global_variable`。
- universal-ctags 的 `member` 被错误映射为全局变量。
- missing-symbol 的本地声明正则允许跨行贪婪匹配，未稳定排除局部变量。

修复措施：

- 解析器增加字符串、字符、行注释和块注释感知的顶层作用域扫描，仅在文件顶层建立全局变量节点。
- ctags 将 `member`、`local` 分别归一化为 `struct_member`、`local_variable`。
- missing-symbol 显式收集函数参数和逐行对象声明，在检索前排除本地符号。
- 解析器升级为 `hybrid-c-parser-v3`，使旧跨任务解析缓存自动失效。

三文件重建后，符号数从 63 降至 44，chunk 从 135 降至 104，图谱边从 316 降至 229，Definition Context 从约 1339 字符降至约 830 字符，错误全局变量降为 0。

### 9.4 同类符号误分类审计

继续审计 tree-sitter、ctags 和正则回退后发现：

- `address.sin_port = htons(...)` 会被宽松正则误识别为 callback binding。
- `struct sockaddr_in address;` 是局部类型使用，却被当作结构体定义卡片。
- ctags 能识别 `union`，但 chunk、关键词检索、图谱类型边和 reranker 没有完整接纳联合体定义。

对应修复：

- callback binding 仅接受右侧为独立标识符且后接初始化器分隔符的形式，排除函数调用表达式和全大写宏/枚举常量。
- tree-sitter 仅把带 body 的 struct/union/enum 作为定义；正则回退只接受真正定义或前向声明。
- callback binding 保留在图谱关系中，但不再作为第一阶段 Definition Context 直接返回。
- 补齐 `union` 的 chunk、关键词权重、类型使用边和 missing-symbol 权重。
- 解析器升级为 `hybrid-c-parser-v5`，再次使旧缓存失效。

### 9.5 后续优先级

1. 静态枚举高风险语句，在第一阶段后只对未覆盖触发位置做小规模补审，优先解决剩余三处重复 `memcpy`，避免继续扩张主提示词。
2. 对同构文件增加文件路径与行号的后端重定位，防止模型沿用另一份相似文件的行号。
3. Definition Context 仅返回真实定义、声明、宏、枚举、全局变量和函数指针类型；只有调用图中存在真实目标时才展示相关函数定义。
4. 当 `finish_reason=length` 时只续审未覆盖文件或触发位置，不重跑已完成内容。
5. 用更多真实项目重复运行 3 至 5 次，统计召回、误报、行号准确率和结果方差，避免针对 DVCP 单一数据集过拟合。

## 10. 2026-06-30 真实代码语义 embedding 与 Qdrant 稳定性优化

本轮将 RAG 向量层从 128 维确定性哈希升级为 Jina Code Embeddings 1.5B，并在单张 A100 40GB 上与 Qwen2.5-Coder-14B-Instruct 并行部署。实现与验证重点如下：

1. embedding 支持 OpenAI 兼容批量接口、query/passage 前缀、输出行数与维度校验；严格模式下服务异常直接失败，不静默回退哈希。
2. embedding signature 纳入后端、模型、维度和集合，避免更换模型后复用旧缓存。
3. 大型数组和常量表仅保存结构化摘要，防止数据表主导 token 与向量语义。
4. embedding 输入限制为 16,000 字符，修复 32,000 字符 C 代码被分成 12,063 tokens、超过 8,192 上下文的问题。
5. Qdrant upsert 每批限制为 128 点，修复 3555 条 1536 维向量单次写入被 400 拒绝的问题。
6. 服务器定向测试为 43 passed；Jansson 索引成功写入 3555 个 Qdrant 点。
7. Jansson 基准中向量 Recall@5 从哈希方案 60% 提升至 100%；无函数名语义改写 Recall@5 与 MRR@10 均为 80%。

完整参数、对象提取结果、图谱指标和剩余风险见《RAG系统架构和评测.md》第 15 节。
